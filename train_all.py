...
parser.add_argument('-cfg', '--cfg', default='FHDC_Net', required=True, type=str,
...
def main():
    ...
    for i, data in enumerate(train_loader, args.start_iter):
        ...
        if current_acc > losses.avg:
            current_acc = losses.avg
            print("best_acc is :",current_acc)
            checkpoint = {
                'iter': i,
                'state_dict': model.state_dict(),
                'optim_dict': optimizer.state_dict(),
            }
            best_model_path = os.path.join(ckpts, 'best_checkpoint.pth')
