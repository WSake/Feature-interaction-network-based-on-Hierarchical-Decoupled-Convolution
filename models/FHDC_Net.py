...
from models.featureinteractiongraph import FeatureInteractionGraph
from models.convcontextbranch import ConvContextBranch, normal_conv_blocks

from models.cgacrf import CGACRF
...
class HDC_Net(nn.Module):
    def __init__(self, in_dim, out_dim, num_filters=32):
        ...
        self.gcn = nn.Sequential(OrderedDict([("FeatureInteractionGraph%02d" % i,
                                               FeatureInteractionGraph(32, 30, kernel=1)
                                               ) for i in range(1)]))
        self.conv51 = normal_conv_blocks(32, 32)

        self.dcn = nn.Sequential(OrderedDict([("ConvContextBranch%02d" % i, ConvContextBranch()) for i in range(1)]))

        self.conv52 = normal_conv_blocks(64, 32)

        self.crffusion_1 = CGACRF(32, 32, 32)
        self.crffusion_2 = CGACRF(32, 32, 32)
        self.crffusion_3 = CGACRF(32, 32, 32)
        self.crffusion_4 = CGACRF(32, 32, 32)
        self.crffusion_5 = CGACRF(32, 32, 32)
        
        self.gam = GAM_Attention(32, 32)
    ...
    def forward(self, x):
        ...
        feat_dcn = self.dcn(xg)
        fcn_conv = self.conv52(feat_dcn)   #([1, 32, 8, 8, 8])  ([1, 32, 8, 8, 8])

        conv_hidden = self.crffusion_1(gcn_conv, fcn_conv)  # CGA-CRF
        ...
        x = self.up_1(x) 
        x = torch.cat((x, conv_hidden), dim=1)
